...
def complete(body,settings,store):
 ...
 start=time.monotonic(); sensitive=contains_secret(body); namespace=body.get("cache_namespace","default"); prompt=normalize_prompt(messages); provider,level=select_provider(messages,settings.providers,settings.routes,body.get("provider")); context={"provider":provider["id"],"model":provider.get("model"),"temperature":body.get("temperature"),"tools":repr(body.get("tools")),"response_format":repr(body.get("response_format"))}; premium=max(settings.providers,key=lambda p:p.get("inputCostPerMillion",0)+p.get("outputCostPerMillion",0)); cache_match=find_match(store.state["cache"],prompt,namespace,settings.similarity_threshold,settings.cache_ttl_seconds,context) if body.get("cache",True) and not sensitive else None; actual=0
 if cache_match:
  entry,score,kind=cache_match; response=copy.deepcopy(entry["response"]); baseline=entry["original_cost"]; response["promptcache"]={"cached":True,"match":kind,"similarity":score,"provider":entry["provider"],"complexity":level}
 else:
  last_error=None
...
def stream_complete(body,settings,store):
    ...
    cache_match=find_match(store.state["cache"],prompt,namespace,settings.similarity_threshold,settings.cache_ttl_seconds,context) if body.get("cache",True) and not sensitive else None
    if cache_match:
        entry,score,kind=cache_match
        ...
        store.add_event({"timestamp":datetime.now(UTC).isoformat(),"cached":True,"provider":entry["provider"],"sensitiveBypass":sensitive,"complexity":level,"actualCost":0,"baselineCost":baseline,"saved":baseline,"latencyMs":round((time.monotonic()-start)*1000)})
        return
